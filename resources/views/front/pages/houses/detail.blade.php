...
@php
    $houses = [
        ['id' => 'h1', 'title' => 'Modern Luxury Apartment',  'location' => 'Kicukiro, Kigali',    'price' => 250000, 'bedrooms' => 4, 'bathrooms' => 3, 'area' => '320 m²',  'type' => 'Apartment', 'image' => asset('assets/img/house1.jpg')],
        ['id' => 'h2', 'title' => 'Villa with Swimming Pool',  'location' => 'Nyarutarama, Kigali', 'price' => 580000, 'bedrooms' => 5, 'bathrooms' => 4, 'area' => '500 m²',  'type' => 'Villa',     'image' => asset('assets/img/house2.jpg')],
        ['id' => 'h3', 'title' => 'Duplex Townhouse',          'location' => 'Gasabo, Kigali',      'price' => 180000, 'bedrooms' => 3, 'bathrooms' => 2, 'area' => '220 m²',  'type' => 'Duplex',    'image' => asset('assets/img/house3.jpg')],
        ['id' => 'h4', 'title' => 'Executive Suite Apartment', 'location' => 'Kimihurura, Kigali',  'price' => 320000, 'bedrooms' => 3, 'bathrooms' => 2, 'area' => '280 m²',  'type' => 'Suite',     'image' => asset('assets/img/house1.jpg')],
        ['id' => 'h5', 'title' => 'Family Residence',          'location' => 'Gisozi, Kigali',      'price' => 150000, 'bedrooms' => 4, 'bathrooms' => 3, 'area' => '350 m²',  'type' => 'Apartment', 'image' => asset('assets/img/house2.jpg')],
        ['id' => 'h6', 'title' => 'Premium Penthouse',         'location' => 'Kacyiru, Kigali',     'price' => 450000, 'bedrooms' => 3, 'bathrooms' => 2, 'area' => '260 m²',  'type' => 'Suite',     'image' => asset('assets/img/house3.jpg')],
    ];

    // $property should come from the controller; fallback to first house for reference
    $property = $property ?? array_merge($houses[0], [
        'images'       => [asset('assets/img/house1.jpg'), asset('assets/img/house2.jpg'), asset('assets/img/house3.jpg')],
        'propertyType' => 'Apartment',
        'furnished'    => true,
        'parking'      => 2,
        'floors'       => 3,
        'description'  => 'Stunning modern apartment with panoramic views, premium finishes, and spacious living areas. Located in the heart of Kicukiro with easy access to amenities.',
        'features'     => [
    ...
    $backLink  = ($property['type'] ?? 'house') === 'plot' ? '/plots' : '/houses';
    $backLabel = ($property['type'] ?? 'house') === 'plot' ? 'Plots'  : 'Houses';
    $gallery   = !empty($property['images']) ? $property['images'] : [$property['image']];

    // Similar = same source list minus current property
    $similar = array_filter($houses, fn($h) => $h['id'] !== $property['id']);
@endphp
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                {{-- Image Gallery --}}
                <div x-data="{ active: 0 }">
                    <div class="rounded-2xl overflow-hidden h-72 md:h-96 bg-gray-100 mb-3 relative">
                        ...
                        {{-- Arrows --}}
                        <button type="button"
                            @click="active = (active - 1 + {{ count($gallery) }}) % {{ count($gallery) }}"
                        ...
                        <button type="button"
                            @click="active = (active + 1) % {{ count($gallery) }}"
                        ...
                        <ul class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            @foreach($property['features'] as $feature)
                                <li class="flex items-start gap-2 text-sm text-gray-500">
                                    <svg class="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
                                    {{ $feature }}
                                </li>
                            @endforeach
                        </ul>
                    ...
                    <div class="space-y-3">
                        ...
                        <a href="tel:{{ $property['contactPhone'] }}"
                           class="flex items-center gap-4 p-3 rounded-xl bg-gray-50 hover:bg-blue-50 transition-colors group">
                            <div class="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center flex-shrink-0">
                                <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 0 1-2.18 2A19.79 19.79 0 0 1 11.6 19a19.5 19.5 0 0 1-6-6A19.79 19.79 0 0 1 2.12 4.18 2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                            </div>
                            <div>
                        ...
                        <a href="mailto:{{ $property['contactEmail'] }}"
                           class="flex items-center gap-4 p-3 rounded-xl bg-gray-50 hover:bg-blue-50 transition-colors group">
                            <div class="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center flex-shrink-0">
                                <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m2 7 10 7 10-7"/></svg>
                            </div>
                            <div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 mt-6">
                        ...
                        <a href="mailto:{{ $property['contactEmail'] ?? 'info@propertiesinvest.com' }}"
                           class="flex-1 flex items-center justify-center gap-2 border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-semibold px-4 py-3 rounded-xl transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m2 7 10 7 10-7"/></svg>
                            Send Email
                        </a>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm sticky top-24">
                    ...
                    <div class="text-center py-4 border-b border-gray-100">
                        ...
                        @if(!empty($property['refNumber']))
                            <p class="text-xs text-gray-400 mt-0.5">Ref: {{ $property['refNumber'] }}</p>
                        @endif
                    </div>
                    ...
                    @foreach($similar as $item)
                        <a href="/houses/{{ $item['id'] }}"
                           class="group bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300">
                            ...
                            <div class="p-4">
                                <span class="text-xs text-blue-600 font-medium bg-blue-50 px-2 py-0.5 rounded-full">{{ $item['type'] }}</span>
                                <h3 class="font-semibold text-gray-900 text-sm mt-2 mb-1 truncate">{{ $item['title'] }}</h3>
                                <p class="text-xs text-gray-400 mb-2 flex items-center gap-1">
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 21c-4.418 0-8-4.03-8-9a8 8 0 1 1 16 0c0 4.97-3.582 9-8 9z"/><circle cx="12" cy="12" r="3"/></svg>
                                    {{ $item['location'] }}
                                </p>
                                <div class="flex items-center justify-between border-t border-gray-100 pt-2 mt-2 text-xs text-gray-400">
